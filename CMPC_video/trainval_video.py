...
def train(max_iter, snapshot, dataset, setname, mu, lr, lr_decay_step, bs, tfmodel_folder,
          finetune, model_name, stop_iter, pre_emb=False, frames=16):
    iters_per_log = 100
    ...
    if pre_emb:
        ...
        model = get_segmentation_model(model_name, mode='train',
                                       vocab_size=vocab_size, start_lr=lr, lr_decay_step=lr_decay_step,
                                       batch_size=bs, finetune=finetune, emb_name=emb_name)
    else:
        model = get_segmentation_model(model_name, mode='train',
                                       vocab_size=vocab_size, start_lr=lr, lr_decay_step=lr_decay_step,
                                       batch_size=bs, finetune=finetune)
    ...
    frames_batch = np.zeros((bs, frames, im_h, im_w, 3), dtype=np.float32)
    ...
    for n_iter in range(max_iter):
        ...
        for n_batch in range(bs):
            ...
            frames = batch['frames'].astype(np.float32)
...
def test(iter, dataset, visualize, setname, dcrf, mu, tfmodel_folder,
    ...
    for n_iter in range(reader.num_batch):
        ...
        for i in range(frames.shape[0]):
            proc_frame = skimage.img_as_ubyte(im_processing.resize_and_pad(frames[i, :, :, :], H, W))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-lrd', type=int, default=800000)  # lr decay step
    ...
    parser.add_argument('-finetune', default=False, action='store_true')  # finetune conv layers in backbone
