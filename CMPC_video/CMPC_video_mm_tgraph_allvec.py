...
class LSTM_model(object):
    ...
    def __init__(self, batch_size=1,
        ...
        self.clip = tf.placeholder(tf.float32, [self.batch_size, self.frames, self.H, self.W, 3])

        self.sample_frames = 5
        self.sample_clip = tf.gather(self.clip, indices=[0, 4, 8, 12, 15], axis=1)
        # self.sample_clip: [B, 5, H, W, 3]
        self.sample_clip = tf.reshape(self.sample_clip, [self.batch_size * self.sample_frames,
                                                         self.H, self.W, 3])
        # self.sample_clip: [B*5, H, W, 3]
        resmodel = deeplab101.DeepLabResNetModel({'data': self.sample_clip}, is_training=False)
        ...
        print("Loading embedding npy at data/{}_emb.npy".format(self.emb_name))
    ...
    def build_graph(self):
        print("\n")
        print("#" * 30)
        print("CMPC_video_mm_tgraph")
        print("Use multimodal feats of 16 frames to build temporal graph.")
        print("Use action lang feat to extract global vector as temporal graph feat.")
        print("Select the gt frame global vector to append to H x W locations.")
        print("#" * 30)
        print("\n")
    ...
    def ea_lang(self, words_parse, words_feat):
        # words_parse: [B, 1, T, 5]
        words_parse_ea = words_parse[:, :, :, 0] + words_parse[:, :, :, 1]
        # words_parse_ea: [B, 1, T]
        ...
        ea_lang_feat = tf.matmul(words_parse_ea, words_feat_reshaped)
        # ea_lang_feat: [B, 1, C]
        ea_lang_feat = tf.nn.l2_normalize(ea_lang_feat, 2)
        ea_lang_feat = tf.reshape(ea_lang_feat, [self.batch_size, 1, 1, self.rnn_size])
        # [B, 1, 1, rnn_size]
        return ea_lang_feat

    def ac_lang(self, words_parse, words_feat):
        # words_parse: [B, 1, T, 5]
        words_parse_ac = words_parse[:, :, :, -2]  # action probability
        # words_parse_ac: [B, 1, T]
        words_feat_reshaped = tf.reshape(words_feat, [self.batch_size, self.num_steps - self.valid_idx[0, 0], self.rnn_size])
        # words_feat_reshaped: [B, T, C]
        ac_lang_feat = tf.matmul(words_parse_ac, words_feat_reshaped)
        # ac_lang_feat: [B, 1, C]
        ac_lang_feat = tf.nn.l2_normalize(ac_lang_feat, 2)
        ac_lang_feat = tf.reshape(ac_lang_feat, [self.batch_size, 1, 1, self.rnn_size])
        # [B, 1, 1, rnn_size]
        return ac_lang_feat
    ...
    def valid_lang(self, words_parse, words_feat):
        ...
        words_parse_valid = words_parse_sum - words_parse[:, :, :, 4]
    ...
    def mutan_head(self, lang_feat, spatial_feat, visual_feat, level=''):
        ...
        # TODO: drop dropout
        spatial_feat = tf.tile(spatial_feat, [self.sample_frames, 1, 1, 1])
        # spatial_feat: [BF, H, W, 8]
        vis_trans = tf.concat([visual_feat, spatial_feat], 3)   # [BF, H, W, C+8]
        vis_trans = self._conv("vis_trans_{}".format(level), vis_trans, 1,
                               self.v_emb_dim+8, self.v_emb_dim, [1, 1, 1, 1])
        vis_trans = tf.nn.tanh(vis_trans)  # [BF, H, W, C]

        # lang feature transform
        # TODO: drop dropout
        lang_feat = tf.tile(lang_feat, [self.sample_frames, 1, 1, 1])
        # lang_feat: [BF, 1, 1, C]
        lang_trans = self._conv("lang_trans_{}".format(level), lang_feat,
                                1, self.rnn_size, self.v_emb_dim, [1, 1, 1, 1])
        lang_trans = tf.nn.tanh(lang_trans)  # [BF, 1, 1, C]

        mutan_feat = vis_trans * lang_trans  # [BF, H, W, C]
        return mutan_feat

    def mutan_fusion(self, lang_feat, spatial_feat, visual_feat, level=''):
        # fuse language feature and visual feature
        # lang_feat: [B, 1, 1, C], visual_feat: [BF, H, W, C], spatial_feat: [B, H, W, 8]
        # output: [BF, H, W, C']
        ...
        fused_feats = tf.stack([head1, head2, head3, head4, head5], axis=4)  # [BF, H, W, C, 5]
        fused_feats = tf.reduce_sum(fused_feats, 4)  # [BF, H, W, C]
    ...
    def build_lang2vis(self, visual_feat, words_feat, lang_feat, words_parse, spatial, level=""):
        # visual_feat: [BF, H, W, C]
        ea_lang_feat = self.ea_lang(words_parse, words_feat)
        mm_feat = self.mutan_fusion(ea_lang_feat, spatial, visual_feat, level=level)
        print("Build MutanFusion Module to get multi-modal features.")

        ac_lang_feat = self.ac_lang(words_parse, words_feat)
        tgraph_feat = self.build_temp_graph(mm_feat, ac_lang_feat,
                                                spatial, level=level)
        print("Build Temporal Graph.")

        gtf_vis_feat = visual_feat[self.sample_frames // 2, :, :, :]
        gtf_vis_feat = tf.reshape(gtf_vis_feat,
                                  [self.batch_size, self.vf_h, self.vf_w, self.v_emb_dim])
        # gtf_vis_feat: [B, H, W, C]
        gtf_mm_feat = mm_feat[self.sample_frames // 2, :, :, :]
        gtf_mm_feat = tf.reshape(gtf_mm_feat,
                                 [self.batch_size, self.vf_h, self.vf_w, self.v_emb_dim])
        # gtf_mm_feat: [B, H, W, C]

        temp_glo_ctx = self.build_temp_ctx(gtf_mm_feat, tgraph_feat, level=level)

        gtf_sgraph_feat = self.build_spa_graph(gtf_mm_feat, words_feat, spatial,
        ...
        feat_all = tf.concat([gtf_vis_feat, gtf_sgraph_feat, temp_glo_ctx, lang_vis_feat, spatial], 3)
        # Feature fusion
        fusion = self._conv("fusion_{}".format(level), feat_all, 1,
                            self.v_emb_dim * 3 + self.rnn_size + 8,
    ...
    def build_lang_parser(self, words_feat):
        ...
        words_parse = self._conv("words_parse_2", words_parse, 1, 500, 5, [1, 1, 1, 1])
        words_parse = tf.nn.softmax(words_parse, axis=3)
        # words_parse: [B, 1, T, 5]
        # Five weights: Entity, Attribute, Static Relation, Action, Unnecessary
        return words_parse
    ...
    def build_temp_graph(self, visual_feat, lang_feat, spatial, level=""):
        # visual_feat: [BF, H, W, C]
        vis_trans = self._conv("tg_vtrans_{}".format(level), visual_feat, 1,
                               self.v_emb_dim, self.v_emb_dim, [1, 1, 1, 1])
        vis_trans = tf.reshape(vis_trans, [self.batch_size * self.sample_frames,
                                           self.vf_h * self.vf_w, self.v_emb_dim])
        # vis_trans: [BF, HW, C]
        # lang_feat: [B, 1, 1, C]
        lang_trans = self._conv("tg_ltrans_{}".format(level), lang_feat, 1,
                                self.rnn_size, self.rnn_size, [1, 1, 1, 1])
        lang_trans = tf.tile(lang_trans, [1, self.sample_frames, 1, 1])  # [B, F, 1, C]
        lang_trans = tf.reshape(lang_trans, [self.batch_size * self.sample_frames, 1, self.rnn_size])
        # lang_trans: [BF, 1, C]
        vis_lang_attn = tf.matmul(lang_trans, vis_trans, transpose_b=True)
        # vis_lang_attn: [BF, 1, HW]
        # normalization for attention maps
        vis_lang_attn = tf.divide(vis_lang_attn, self.v_emb_dim ** 0.5)
        vis_lang_attn = tf.nn.softmax(vis_lang_attn, axis=2)

        vis_resh = tf.reshape(visual_feat, [self.batch_size * self.sample_frames,
                                            self.vf_h * self.vf_w, self.v_emb_dim])
        # vis_resh: [BF, HW, C]
        tgraph_feat = tf.matmul(vis_lang_attn, vis_resh)
        # tgraph_feat: [BF, 1, C]
        tgraph_feat = tf.reshape(tgraph_feat, [self.batch_size, 1, self.sample_frames, self.v_emb_dim])
        # tgraph_feat: [B, 1, F, C]

        # obtain adjacency matrix
        tg_query = self._conv("tg_query_{}".format(level), tgraph_feat, 1,
                              self.v_emb_dim, self.v_emb_dim, [1, 1, 1, 1])
        tg_query = tf.reshape(tg_query, [self.batch_size, self.sample_frames, self.v_emb_dim])
        tg_key = self._conv("tg_key_{}".format(level), tgraph_feat, 1,
                            self.v_emb_dim, self.v_emb_dim, [1, 1, 1, 1])
        tg_key = tf.reshape(tg_key, [self.batch_size, self.sample_frames, self.v_emb_dim])
        tg_adj = tf.matmul(tg_query, tg_key, transpose_b=True)
        # tg_adj: [B, F, F]
        tg_adj = tf.divide(tg_adj, self.v_emb_dim ** 0.5)
        tg_adj = tf.nn.softmax(tg_adj, axis=2)

        # graph convolution
        tgraph_feat = self.graph_conv(tgraph_feat, self.sample_frames, self.v_emb_dim,
                                      tg_adj, graph_name="temp_graph", level=level)
        # tgraph_feat: [B, 1, F, C]
        tgraph_feat = tf.nn.l2_normalize(tgraph_feat, 3)

        return tgraph_feat

    def build_temp_ctx(self, mm_feat, ctx_vec, level=""):
        # mm_feat: [B, H, W, C]
        mm_trans = self._conv("mm_trans_{}".format(level), mm_feat, 1,
                              self.v_emb_dim, self.v_emb_dim, [1, 1, 1, 1])
        mm_trans = tf.reshape(mm_trans, [self.batch_size, self.vf_h * self.vf_w,
                                         self.v_emb_dim])
        # mm_trans: [B, HW, C]
        # ctx_vec: [B, 1, F, C]
        ctx_trans = self._conv("ctx_trans_{}".format(level), ctx_vec, 1,
                               self.v_emb_dim, self.v_emb_dim, [1, 1, 1, 1])
        ctx_trans = tf.reshape(ctx_trans, [self.batch_size, self.sample_frames, self.v_emb_dim])
        # ctx_trans: [B, F, C]
        mm_ctx_attn = tf.matmul(mm_trans, ctx_trans, transpose_b=True)
        # mm_ctx_attn: [B, HW, F]
        mm_ctx_attn = tf.divide(mm_ctx_attn, self.v_emb_dim ** 0.5)
        mm_ctx_attn = tf.nn.softmax(mm_ctx_attn, axis=2)

        ctx_resh = tf.reshape(ctx_vec, [self.batch_size, self.sample_frames, self.v_emb_dim])
        # ctx_resh: [B, F, C]
        glo_ctx_feat = tf.matmul(mm_ctx_attn, ctx_resh)
        # glo_ctx_feat: [B, HW, C]
        glo_ctx_feat = tf.reshape(glo_ctx_feat, [self.batch_size, self.vf_h, self.vf_w, self.v_emb_dim])
        # glo_ctx_feat: [B, H, W, C]
        glo_ctx_feat = tf.nn.l2_normalize(glo_ctx_feat, 3)
    ...
    def _deconv(self, name, x, filter_size, in_filters, out_filters, strides, output_shape):
        with tf.variable_scope(name):
            w = tf.get_variable('DW',[filter_size, filter_size, out_filters, in_filters],
                                initializer=tf.contrib.layers.xavier_initializer_conv2d())
            b = tf.get_variable('biases', out_filters, initializer=tf.constant_initializer(0.))
            return tf.nn.conv2d_transpose(x, w, output_shape, strides, padding='VALID') + b
    ...
    def train_op(self):
        if self.finetune:
            tvars = [var for var in tf.trainable_variables() if var.op.name.startswith('text_objseg')
                     ...
                     or var.name.startswith('res3') or var.name.startswith('video2image')]
        else:
            tvars = [var for var in tf.trainable_variables() if var.op.name.startswith('text_objseg')
                     or var.name.startswith('video2image')]
...
if __name__ == "__main__":
    model = LSTM_model(num_steps=20, mode='train',
                       vocab_size=12112, start_lr=0.01, batch_size=1)
